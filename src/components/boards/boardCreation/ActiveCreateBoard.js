...
class ActiveCreateBoard extends Component {
	...
	render() {
		const { cancelCreatingBoard } = this.pops;

		return (
			<Wrapper>
				<TopWrapper>
					<Title>Creating a board</Title>
					<CloseBoardIcon onClick={() => cancelCreatingBoard()} src={require('~/assets/closeIcon.svg')} /> 
				</TopWrapper>

				<BodyWrapper>
					<BoardNamingTitle>What shall we call the board?</BoardNamingTitle>
					<BoardTitleForm onSubmit={this.submit} cancelAction={cancelCreatingBoard}/>
				</BodyWrapper>
			</Wrapper>
		)
	}
